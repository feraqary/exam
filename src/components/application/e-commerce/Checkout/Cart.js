...
const Increment = ({ itemId, quantity, updateQuantity }) => {
  ...
  return (
    <ButtonGroup size="large" variant="text" color="inherit" sx={{ border: '1px solid', borderColor: 'grey.400' }}>
      ...
      <Button aria-label="decrease" key="one" onClick={decrementHandler} sx={{ pl: 0.75, pr: 0.75, minWidth: '0px !important' }}>
                  ...
                  <TableRow key={index} sx={{ '&:last-of-type td, &:last-of-type th': { border: 0 } }}>
                    <TableCell component="th" scope="row">
                      <Grid container alignItems="center" spacing={2}>
                        <Grid item>
                          <Avatar size="md" variant="rounded" src={row.image ? `${prodImage}/${row.image}` : ''} alt="product images" />
                        </Grid>
                        <Grid item>
                          <Stack spacing={0}>
