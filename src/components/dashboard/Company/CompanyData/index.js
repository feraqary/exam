...
import { useUpdateCompanyRankMutation, useUpdateCompanyStatusMutation } from 'store/services/company/companyApi';
...
function CompanyData({ fetchData, title }) {
  const [pagination, setPagination] = useState({
    pageIndex: 0,
    pageSize: 5
  });
  const { data: companyData, isError, error, isLoading, isFetching } = fetchData(pagination);
      ...
      Cell: ({ renderedCellValue, row }) => {
        return (
          <Box sx={{ display: 'flex', alignItems: 'center' }}>
            <TableSelectorOption value={row.original.CompanyRank} formData={formData} func={useUpdateCompanyRankMutation} />
          </Box>
        );
      }
    },
        ...
        <Grid container spacing={gridSpacing}>
          <Grid item xs={12}>
            <Table
              ...
              data={error ? [] : companyData?.data || []}
              loading={isLoading}
              pagination={pagination}
              setPagination={setPagination}
              isFetching={isFetching}
              rowCount={companyData?.Total}
            />
          </Grid>
        </Grid>
...
CompanyData.getLayout = function getLayout(page) {
