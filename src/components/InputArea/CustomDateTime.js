...
const CustomDateTime = memo(({ style, label, readonly, helperText, value, setValue, required, name, id, helperInfo, func, ...rest }) => {
...
export const NormalCustomDateTime = memo(({ style, label, helperText, value, setValue, required, helperInfo, func, ...rest }) => {
  return (
    <InputLayout label={label} style={{ xs: style.x, lg: style.lg }} helperText={helperText} required={required} helperInfo={helperInfo}>
      <LocalizationProvider dateAdapter={AdapterDateFns}>
        <MobileDatePicker
          {...rest}
          label={label}
